...
- Shared config at root: `vite.config.mjs`, `eslint.config.mjs`, `biome.json`, `.prettierrc`
...
- Lint (auto-fix / check): `pnpm lint` / `pnpm lint:no-fix`
- Format (Prettier) / Biome: `pnpm format`, `pnpm biome:check`, `pnpm biome:fix`
...
- Strings: repository defaults (Prettier config)
