...
- **Compatibility Tests** (`.github/workflows/compatibility-tests.yml`)
  - Triggers: manual dispatch (with flags), reusable `workflow_call`, tag pushes (`v*`), and release publication.
  - Matrix lanes:
    - Release/tag triggers automatically run the latest Node LTS lane (`node24` combo) to block publishes on regressions.
    - Manual runs let you opt into the curated suites via inputs (`run_latest`, `run_oldest`).
...
- Release/tag invocations only execute the latest LTS lane and complete in ~8 minutes.
