...
class VersionTester {
  ...
  async testVersion(category, packageName, version) {
    try {
      ...
      if (category === 'devtools') {
        ...
        testPackageJson.devDependencies['react-dom'] = LATEST_BASE_VERSIONS['react-dom'];
        testPackageJson.devDependencies.typescript = LATEST_BASE_VERSIONS.typescript;
      }
      ...
      if (category === 'devtools' && 
          (packageName.includes('eslint') || packageName === 'prettier' || packageName === '@biomejs/biome')) {
        try {
          ...
          if (lintError.message.includes('command not found') || 
  ...
  restorePackageJson() {
    if (this.originalPackageJson) {
      ...
      try {
        execSync('npm install --silent --no-audit --no-fund', { stdio: 'pipe' });
      } catch (error) {
  ...
  generateMarkdownReport(report) {
    ...
    fs.writeFileSync('version-compatibility-tests/COMPATIBILITY_REPORT.md', markdown);
  }
}
