...
function ensureCounter(
  map: Map<string, OutcomeBreakdown>,
  key: string,
): OutcomeBreakdown {
  if (!map.has(key)) {
...
export function aggregateResults(results: ScenarioResult[]): AggregatedSummary {
  ...
  for (const scenario of results) {
    const outcome: ScenarioResultOutcome =
      scenario.outcome ||
      (scenario.steps?.build && scenario.steps?.pin_and_build
        ? "PASS"
        : "FAIL");
    ...
    if (linterInfo) {
      if (linterInfo.tool === "biome" && linterInfo.version)
        lintSets.biome.add(linterInfo.version);
...
export function renderMarkdown(
  ...
  if (lintSets.biome.size)
    lintParts.push(`biome: ${Array.from(lintSets.biome).sort().join(", ")}`);
  if (lintSets.eslint.size)
    lintParts.push(`eslint: ${Array.from(lintSets.eslint).sort().join(", ")}`);
  if (lintSets.prettier.size)
    lintParts.push(
      `prettier: ${Array.from(lintSets.prettier).sort().join(", ")}`,
    );
  if (lintParts.length) {
    md += `### Linter Versions\n\n`;
