...
export function createRuntimeController({
  ...
  logsRoot,
  logPrefix,
}: RuntimeControllerOptions): RuntimeController {
  const originalNodeVersion = process.version.startsWith("v")
  ...
  function ensureRuntime(runtime: RuntimeDescriptor): boolean {
    ...
    if (runtime.tool === "node") {
      ...
      const res = runCommand(`pnpm env use --global ${runtime.versionSpec}`, {
        cwd: rootDir,
        logFile,
      });
      if (!res.ok) {
        const error = new Error(
          `Failed to activate Node runtime ${runtime.label}`,
    ...
    console.warn(
      `${logPrefix} Runtime ${runtime.label} (${runtime.tool}) is not implemented yet; skipping scenarios`,
    );
    activeRuntimeKey = key;
    return false;
  }
  ...
  function restoreRuntime(): void {
    ...
    const res = runCommand(`pnpm env use --global ${restoreNodeVersion}`, {
      cwd: rootDir,
      logFile,
    });
    if (!res.ok) {
      console.warn(
        `${logPrefix} Failed to restore Node runtime ${restoreNodeVersion}: ${res.out}`,
      );
    } else {
