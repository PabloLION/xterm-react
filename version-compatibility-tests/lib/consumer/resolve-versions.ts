...
export function pickLatestForMajor(
  versions: string[],
  major: string,
): string | null {
...
export function resolveConsumerVersions(
  args: ResolveArgs,
  ...
  const versions: Record<string, string> = {
    ...
    "@types/react":
      args.typesReact || getLatestForMajor("@types/react", reactMajor),
    "@types/react-dom":
      args.typesReactDom || getLatestForMajor("@types/react-dom", reactMajor),
    vite: args.vite || getLatest("vite"),
    "@vitejs/plugin-react":
      args.pluginReact || getLatest("@vitejs/plugin-react"),
  };
