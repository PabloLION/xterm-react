...
const PRETTIER_VERSIONS_TO_TEST = [
  ...
  { name: 'Prettier 3.3 (Current)', version: '^3.3.0', pluginVersion: '^5.5.4' }
];
...
class ESLintVersionTester {
  ...
  async testESLintVersion(config) {
    try {
      ...
      try {
        ...
        var lintSuccess = !lintError.message.includes('command not found') && 
  ...
  async testPrettierVersion(config) {
    try {
      ...
      testPackageJson.devDependencies['eslint-plugin-prettier'] = config.pluginVersion;
      ...
      try {
        ...
        var prettierSuccess = !prettierError.message.includes('command not found') && 
  ...
  generateReport() {
    ...
    const prettierResults = this.results.filter(r => r.category === 'Prettier');
