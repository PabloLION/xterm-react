...
class BiomeVersionTester {
  ...
  async testAllVersions() {
    ...
    if (fs.existsSync('biome.json')) {
      this.originalBiomeConfig = JSON.parse(fs.readFileSync('biome.json', 'utf8'));
    }
  ...
  async testBiomeVersion(config) {
    try {
      ...
      try {
        ...
        var checkSuccess = !checkErr.message.includes('command not found') && 
      ...
      try {
        ...
        var formatSuccess = !formatErr.message.includes('command not found') && 
      ...
      try {
        ...
        var migrateSuccess = migrateErr.message.includes('no migration needed') ||
  ...
  restoreOriginalFiles() {
    ...
    if (this.originalBiomeConfig) {
      fs.writeFileSync('biome.json', JSON.stringify(this.originalBiomeConfig, null, 2));
    }
